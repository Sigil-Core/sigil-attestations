...
- Both APIs require a strict CC-1 signed Warrant, derive the policy hash from its unsigned bytes through the `@sigilcore/warrant-core` canonicalizer stamped as `@sigilcore/warrant-core@0.2.3` (`CANONICALIZER_VERSION`), and reject bundles larger than 1 MiB or Warrants larger than 256 KiB.
...
`CANONICALIZER_VERSION` identifies canonicalization behavior, not the installed npm version, so it does not track the `package.json` pin and is not expected to match it. It advances when a `@sigilcore/warrant-core` release changes canonicalization output for a policy both releases accept, or widens the set of policies that can be canonicalized at all. In either case the previous value moves to `HISTORICAL_CANONICALIZER_VERSION` so already-issued bundles stay verifiable. The pin may therefore sit ahead of the identifier.

It sits ahead today: `0.4.0` is pinned and the identifier is `0.2.3`. Measured against a 20-policy corpus drawn from the shipped `sigil-open-framework` examples, `sigil-sign` fixtures, and this package's own test Warrant, every policy accepted by both releases canonicalizes to byte-identical output and the same policy hash under `0.2.1`, `0.2.3`, `0.2.4`, `0.3.0`, and `0.4.0`. Thirteen policies fall in that shared domain and none of them differs. No already-issued bundle changes hash and none needs re-issuance.

What `0.4.0` does change is the accepted policy-version range. `0.2.3` supports Policy `0.x`, `1.x`, `2.0.x`, and `2.1.x`; `0.3.0` adds `2.2.x`; `0.4.0` adds `2.3.x`, along with a compiled response-policy surface and a `response.deny_string` custom rule that earlier releases have no emission path for. Two policies in the corpus, `developer-toolkit/warranty-policy.md` and `examples/mcp-server-agent/warranty.md`, are Policy `2.3.0` and are canonicalized by `0.4.0` after being rejected outright by every earlier release. A bundle built from such a policy and stamped `0.2.3` would name a canonicalizer that could not have produced it, so the identifier must advance before this package issues a bundle for any Policy `2.2` or `2.3` Warrant. It is accurate for the Policy `2.1` and earlier Warrants issued to date.

The test for whether the identifier needs to move is behavioral, never a version-string comparison and never a tarball diff alone: canonicalize a corpus under both releases, compare output bytes across the shared accepted domain, then compare the accepted domains themselves.
