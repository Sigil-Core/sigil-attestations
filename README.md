...
- Both APIs require a strict CC-1 signed Warrant, derive the policy hash from its unsigned bytes through the pinned `@sigilcore/warrant-core` canonicalizer, identified in the envelope as `@sigilcore/warrant-core@0.4.0` (`CANONICALIZER_VERSION`), and reject bundles larger than 1 MiB or Warrants larger than 256 KiB.
