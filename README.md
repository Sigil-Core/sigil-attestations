...
`CANONICALIZER_VERSION` identifies canonicalization behavior, not the installed npm version, so it does not track the `package.json` pin and is not expected to match it. It advances only when a `@sigilcore/warrant-core` release changes canonicalization output, at which point the previous value moves to `HISTORICAL_CANONICALIZER_VERSION` so already-issued bundles stay verifiable. The pin may therefore sit ahead of the identifier. It does today: `0.2.4` is pinned, and its published tarball is byte-identical to `0.2.3` across every non-documentation file, so it materializes no new proof-bundle envelope and the identifier correctly stays at `0.2.3`. The test for whether the identifier needs to move is a tarball diff of the two releases, never a version-string comparison.
